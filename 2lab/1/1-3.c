...
int main() {
    ...
    if(signal(SIGALRM, inter_handler) == SIG_ERR)
    {
    ...
    if (pid1 > 0) 
    {
        ...
        if (pid2 > 0) 
        {
            ...
            if(parent_recv_sig == SIGINT || parent_recv_sig == SIGQUIT)
                printf("Parent: received SIGINT or SIGQUIT\n");
            else 
